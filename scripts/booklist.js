...
const bookList = (function(){
  ...
  function generateItemElement(item) {
    if (item.expanded) {
      return `
      <li class="js-bookmark-item__expanded container" data-item-id="${item.id}">
        <div class="name-rate">
        ...
        <div class="expandedlist">
          <form id="js-edit-item-desc">
            <textarea class="bookmark-desc" type="text" aria-label="Editable description" name="desc" cols="40" rows="3" required>${item.desc}</textarea>
    ...
    <li class="js-bookmark-item container" data-item-id="${item.id}">
      <div class="name-rate">
        <span name="${item.title}" class="list-name">${item.title} (click to expand) </span>
        <span name="${item.rating}" class="list-rating rating">${item.rating} Stars</span>
      </div>
    </li>
    `;}
  }
  ...
  function generateEditBox(){
    return `
      <form id="js-add-entry" class="container add-form" role="form">
        ...
        <input type="text" required aria-label="Enter description" name="desc" class="desc-entry" placeholder="Enter description here">
